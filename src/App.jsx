...
function buildBackupPayload({ profiles, entries, goals, libraryCatalog, cloudConfig }) {
  return {
    app: 'suivi-spirituel-pwa',
    version: 2,
...
function App() {
  ...
  const [cloudConfig, setCloudConfig] = useState(defaultCloudConfig);
  ...
  useEffect(() => {
    (async () => {
      const [savedProfiles, savedEntries, savedGoals, savedActiveId, savedCatalog, savedCloudConfig] = await Promise.all([
        db.getItem('profiles'),
        db.getItem('entries'),
        db.getItem('goals'),
        db.getItem('activeProfileId'),
        db.getItem('libraryCatalog'),
        db.getItem('cloudConfig')
      ]);
      ...
      setCloudConfig({ ...defaultCloudConfig, ...(savedCloudConfig || {}) });
  ...
  useEffect(() => { if (ready) db.setItem('profiles', profiles); }, [profiles, ready]);
  useEffect(() => { if (ready) db.setItem('entries', entries); }, [entries, ready]);
  useEffect(() => { if (ready) db.setItem('goals', goals); }, [goals, ready]);
  useEffect(() => { if (ready) db.setItem('activeProfileId', activeProfileId); }, [activeProfileId, ready]);
  useEffect(() => { if (ready) db.setItem('libraryCatalog', libraryCatalog); }, [libraryCatalog, ready]);
  useEffect(() => { if (ready) db.setItem('cloudConfig', cloudConfig); }, [cloudConfig, ready]);
  ...
  const monthlyStats = useMemo(() => monthOptions.map((key) => ({
    key,
  ...
  const backupPayload = useMemo(
    () => buildBackupPayload({ profiles, entries, goals, libraryCatalog, cloudConfig }),
    [profiles, entries, goals, libraryCatalog, cloudConfig]
  );

  function setCloudField(key, value) {
    setCloudConfig((prev) => ({ ...prev, [key]: value }));
  }
  ...
  async function saveEntry() {
    ...
    if (cloudConfig.autoBackupOnSave && googleToken) {
      setTimeout(() => {
        uploadBackupToDrive().catch(() => null);
      }, 250);
    }
  }
  ...
  function exportJson() {
    downloadBlob(
      `suivi-spirituel-backup-${todayISO()}.json`,
      new Blob([JSON.stringify(backupPayload, null, 2)], { type: 'application/json' })
    );
    setStatus('Sauvegarde JSON exportée.');
  }

  async function shareBackupFile() {
    const blob = new Blob([JSON.stringify(backupPayload, null, 2)], { type: 'application/json' });
    const file = new File([blob], `suivi-spirituel-backup-${todayISO()}.json`, { type: 'application/json' });
    if (navigator.share && navigator.canShare?.({ files: [file] })) {
      try {
        await navigator.share({ title: 'Sauvegarde suivi spirituel', files: [file] });
    ...
    setStatus('Partage de fichier non disponible, téléchargement lancé.');
  }

  function exportCsv() {
    const rows = [
      ['Profil', 'Date', 'Meditation', 'Priere', 'Chapitres', 'Pages', 'JeuneJours', 'TypeJeune', 'Evangelisation', 'PG', 'Confession', 'Retraite', 'EtudeJC']
    ];
  ...
  function importBackup(event) {
    ...
    reader.onload = () => {
      try {
        ...
        applyImportedBackup(parsed, 'Sauvegarde importée.');
      } catch {
  ...
  function applyImportedBackup(parsed, successMessage) {
    if (Array.isArray(parsed.profiles)) setProfiles(parsed.profiles);
    if (Array.isArray(parsed.entries)) setEntries(parsed.entries);
    if (parsed.goals) setGoals({ ...defaultGoals, ...parsed.goals });
    if (Array.isArray(parsed.libraryCatalog)) setLibraryCatalog(parsed.libraryCatalog);
    if (parsed.cloudConfig) setCloudConfig((prev) => ({ ...prev, ...parsed.cloudConfig }));
    const nextProfiles = Array.isArray(parsed.profiles) && parsed.profiles.length ? parsed.profiles : profiles;
    const nextActive = nextProfiles[0]?.id || activeProfileId;
    setActiveProfileId(nextActive);
    setForm({ ...emptyEntry(), profileId: nextActive });
  ...
  function disconnectGoogle() {
    if (window.google?.accounts?.oauth2 && googleToken) {
      window.google.accounts.oauth2.revoke(googleToken, () => null);
    }
    setGoogleToken('');
    setGoogleEmail('');
    tokenClientRef.current = null;
    setStatus('Connexion Google retirée sur cet appareil.');
  }
  ...
  async function findDriveBackupFileId(token) {
    ...
    const response = await fetch(`https://www.googleapis.com/drive/v3/files?spaces=appDataFolder&fields=files(id,name,modifiedTime)&q=${query}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
    if (!response.ok) throw new Error('Recherche Drive impossible');
  ...
  async function uploadBackupToDrive() {
    try {
      ...
      const existingFileId = await findDriveBackupFileId(token);
      const metadata = existingFileId
        ? { name: DRIVE_FILE_NAME }
        : { name: DRIVE_FILE_NAME, parents: ['appDataFolder'] };
      const formData = new FormData();
      formData.append('metadata', new Blob([JSON.stringify(metadata)], { type: 'application/json' }));
      formData.append('file', new Blob([JSON.stringify(buildBackupPayload({ profiles, entries, goals, libraryCatalog, cloudConfig }), null, 2)], { type: 'application/json' }));
      ...
      if (!response.ok) throw new Error('Échec de sauvegarde Drive');
      const timestamp = new Date().toISOString();
      setCloudConfig((prev) => ({ ...prev, lastCloudBackupAt: timestamp }));
      setStatus('Sauvegarde envoyée sur Google Drive.');
    } catch (error) {
      setStatus(`Sauvegarde cloud impossible: ${error.message}`);
    } finally {
      setCloudBusy(false);
    }
  }
  ...
  async function restoreBackupFromDrive() {
    try {
      setCloudBusy(true);
      const token = await ensureGoogleToken();
      const fileId = await findDriveBackupFileId(token);
      if (!fileId) {
        setStatus('Aucune sauvegarde cloud trouvée.');
        return;
      }
      const response = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?alt=media`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (!response.ok) throw new Error('Lecture Drive impossible');
      const parsed = await response.json();
      const timestamp = new Date().toISOString();
      applyImportedBackup(parsed, 'Sauvegarde cloud restaurée.');
      setCloudConfig((prev) => ({ ...prev, lastCloudRestoreAt: timestamp }));
    } catch (error) {
      setStatus(`Restauration cloud impossible: ${error.message}`);
    } finally {
      setCloudBusy(false);
    }
  }
      ...
      <section className="hero-grid">
        <div className="hero-card gradient-card">
          <div className="hero-content">
            ...
            <p>Journal quotidien, tableau de bord, bibliothèque mémorisée, export, partage, supervision locale et sauvegarde cloud Google Drive.</p>
      ...
      <main className="content">
        {tab === 'home' && (
          <div className="stack">
            ...
            <Card title="Sauvegarde protégée" icon={<Cloud size={18} />}>
              <div className="grid two">
                <LabelValue label="Dernière sauvegarde cloud" value={formatDateTimeFR(cloudConfig.lastCloudBackupAt)} />
                <LabelValue label="Dernière restauration" value={formatDateTimeFR(cloudConfig.lastCloudRestoreAt)} />
                <LabelValue label="Google" value={googleEmail || (googleToken ? 'Connecté' : 'Non connecté')} />
                <LabelValue label="Auto-sauvegarde" value={cloudConfig.autoBackupOnSave ? 'Activée' : 'Désactivée'} />
              </div>
                ...
                {form.bibleReadings.map((item, index) => {
                  ...
                  return (
                    <div className="row-card" key={index}>
                      ...
                      <div className="grid two">
                        <input className="input" type="number" min="1" max={maxChapters || undefined} placeholder="Chap. début" value={item.start} onChange={(e) => updateBibleReading(index, { start: e.target.value })} />
                        <input className="input" type="number" min="1" max={maxChapters || undefined} placeholder="Chap. fin" value={item.end} onChange={(e) => updateBibleReading(index, { end: e.target.value })} />
                      </div>
        ...
        {tab === 'dashboard' && (
          <div className="stack">
            ...
            <Card title="Vue responsable locale" icon={<Users size={18} />}>
              ...
              <p className="footnote">Cette vue supervise les profils stockés sur ce téléphone. La sauvegarde cloud Google Drive sert ici de copie de sécurité et de restauration.</p>
            </Card>
          </div>
        )}
        ...
        {tab === 'library' && (
          <div className="stack">
            ...
            <Card title="Sauvegarde et partage" icon={<Download size={18} />}>
              <div className="quick-actions">
                <button className="btn primary" onClick={exportJson}><Download size={16} /> Export JSON</button>
                <button className="btn secondary" onClick={shareBackupFile}><Share2 size={16} /> Partager JSON</button>
          ...
          <div className="stack">
            ...
            <Card title="PWA et installation" icon={<Smartphone size={18} />}>
              <ul className="plain-list">
                ...
                <li>Le stockage local fonctionne hors ligne, mais une sauvegarde cloud évite les pertes.</li>
...
function StatCard({ label, value, goal, unit, compareMinutes = false }) {
  const numericValue = compareMinutes ? Math.round((timeToMinutes(value) || 0) / 60) : Number(value) || 0;
  const percent = goal ? Math.min(100, Math.round((numericValue / Number(goal)) * 100)) : 0;
  return (
    <div className="stat-card">
      ...
      <div className="muted inline-note">Objectif: {goal} {unit}</div>
    </div>
  );
}
...
function GoalInput({ label, value, onChange }) {
  return (
    <div>
      ...
      <input className="input" type="number" min="0" step="1" value={value} onChange={(e) => onChange(e.target.value)} />
    </div>
  );
}
